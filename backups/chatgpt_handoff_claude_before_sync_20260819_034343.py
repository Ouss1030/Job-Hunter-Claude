...
from config.ai_generation import (
    CANDIDATE_TRUTH as CONFIG_CANDIDATE_TRUTH,
...
# La vérité candidat n'est PAS redéfinie ici.
# config/ai_generation.py en est la source unique : la dupliquer ferait
# diverger silencieusement les faits qui ne doivent jamais être faux
# (diplômes, années d'expérience, outils non maîtrisés).
BASE_CV_PREFERENCE = DEFAULT_BASE_CV_NAME

# Règles de rédaction. Ce sont des consignes, pas des faits : elles ont
# donc leur place ici, contrairement à la vérité candidat.
HARD_TRUTH_RULES = [
    ...
    "Ne jamais revendiquer : " + ", ".join(sorted(FORBIDDEN_CANDIDATE_CLAIM_TERMS)) + ".",
]

CANDIDATE_TRUTH = {
    **CONFIG_CANDIDATE_TRUTH,
    "hard_truth_rules": HARD_TRUTH_RULES,
    "forbidden_claim_terms": sorted(FORBIDDEN_CANDIDATE_CLAIM_TERMS),
}
...
def field(item, *names, default=None):
    ...
    On lit du suffixe le plus récent au nom de base, pour rester lisible
    par un pool plus ancien comme par un pool plus récent.
    """
    for name in names:
        value = item.get(name)
...
def select_items(payload, actions, start=1, limit=None):
    ...
    selected.sort(
        key=lambda x: (
            int(field(x, "pool_rank_v12", "pool_rank_v11", "pool_rank",
                      default=999999)),
            -float(field(x, "final_score_v12", "final_score_v11",
                         "final_score", default=0)),
        )
    )
...
def compact_job(item):
    return {
        ...
        "pool_rank": field(item, "pool_rank_v12", "pool_rank_v11", "pool_rank"),
        ...
        "priority": field(item, "priority_v12", "priority_v11", "priority"),
        "final_score": field(item, "final_score_v12", "final_score_v11", "final_score"),
        "recommended_action": field(item, "recommended_action_v12", "recommended_action_v11", "recommended_action"),
        ...
        # candidate_truth et application_instructions vivent une seule
        # fois à la racine du bundle (candidate_truth.json / manifest.json),
        # qui sont aussi copiés dans chaque ZIP de lot.
    }
...
--------------------
À partir de l'offre (job.json) et de la vérité candidat
(candidate_truth.json, à la racine de ce paquet) :
...
def export_handoff(actions, start=1, limit=None, chunk_size=DEFAULT_CHUNK_SIZE):
    # Du schéma le plus récent au plus ancien. Épingler « V1.2 requis »
    # ferait échouer ce module au prochain pool sans rien y gagner :
    # select_items() et compact_job() lisent déjà les champs suffixés
    # avec repli sur les noms de base.
    source = None
    for motif in ("final_application_pool_v12_*.json",
                  "final_application_pool_v11_*.json",
                  "final_application_pool_v1_*.json"):
        source = latest_file(motif)
        if source is not None:
            break

    if source is None:
        raise RuntimeError(
            "Aucun final_application_pool_*.json trouvé dans exports/logs.\n"
            "Lance d'abord : python -m applications.final_application_pool"
        )

    payload = load_json(source)

    version = str(payload.get("pool_version") or "?")
    if version != "1.2":
        print(f"⚠️  Pool en version {version} (V1.2 attendue) : "
              f"les champs recalculés récents peuvent manquer.")
