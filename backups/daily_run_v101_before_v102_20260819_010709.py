...
def print_preflight(result):
    print("=" * 100)
    print("JOB HUNTER DAILY RUN V1.0 - PREFLIGHT")
    print("=" * 100)
    print()
    ...
    print()
    if result["ok"]:
        print("✅ PREFLIGHT DAILY RUN V1.0 VALIDÉ.")
    else:
...
def validate_refresh_json(path, expected_count=None):
    ...
    statuses = {}
    for row in payload:
        status = clean_text(row.get("status") or row.get("refresh_status") or "UNKNOWN")
...
def write_manifest_txt(path, manifest):
    lines = []
    lines.append("JOB HUNTER DAILY RUN V1.0")
    lines.append("=" * 100)
...
def print_final_summary(manifest):
    print()
    print("=" * 100)
    print("JOB HUNTER DAILY RUN V1.0 - BILAN")
    print("=" * 100)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Job Hunter Belgium Daily Run V1.0"
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--no-handoff",
        action="store_true",
        help="Arrêter le pipeline après Final Pool.",
    )
    parser.add_argument(
        "--stop-after",
        choices=STEP_ORDER,
