"""
JOB HUNTER BELGIUM
APPLICATION POST-REFRESH RECHECK - VERSION 1.0
...
def mandatory_master_block(text):
    ...
    # Alternatives Bachelor/Master : pas de blocage.
    alternatives = [
        "bachelor ou master",
        "bachelor ou d'un master",
        "bachelier ou master",
        "bachelier ou d'un master",
        "bachelor or master",
        "bachelor of master",
        "master of bachelordiploma",
        "master- of bachelordiploma",
    ]
    if any(a in n for a in alternatives):
        return False

    hard_patterns = [
        r"\bmaster diploma\b",
        r"\bmasterdiploma\b",
        r"\bdiplome de master\b",
        r"\bmaster en [a-z]",
        r"\bmaster in [a-z]",
    ]
    return any(re.search(p, n) for p in hard_patterns)


def explicit_student_role(text):
    n = normalize(text)
    markers = [
        "student quality controller",
    ...
    return any(normalize(m) in n for m in markers)
...
def strong_language_gap(text):
    ...
    if any(normalize(p) in n for p in dutch_strong) and CANDIDATE["languages"]["nl"] == "B1":
        warnings.append("Néerlandais demandé à un niveau professionnel/aisé ; candidat B1")
    ...
    if any(normalize(p) in n for p in english_strong) and CANDIDATE["languages"]["en"] == "B1":
        warnings.append("Anglais professionnel/bon niveau demandé ; candidat B1")
...
def data_experience_or_degree_stretch(item, text):
    ...
    if CANDIDATE["data_professional_years"] <= 0:
        if any(p in n for p in [
            ...
            warnings.append("Expérience professionnelle Data/BI demandée ; candidat surtout académique/projet")

    if any(p in n for p in [
        "bachelor ou d'un master en informatique",
        "bachelor ou master en informatique",
        "bachelor in informatica",
        "master en informatique",
    ]):
        warnings.append("Diplôme explicitement orienté informatique ; profil candidat adjacent via BDA/chimie")

    stack_terms = {
        "talend", "apache hop", "tableau", "oracle", "pl/sql", "sap business object"
    }
    mentioned = sorted(skill for skill in stack_terms if skill in n)
    missing = [skill for skill in mentioned if skill not in CANDIDATE["known_skills"]]
    if len(missing) >= 2:
        warnings.append("Stack spécifique non démontrée : " + ", ".join(missing))
...
def export_results(results):
    ...
    s = summary(results)
    lines = [
        "APPLICATION POST-REFRESH RECHECK V1",
        "=" * 78,
        f"Total              : {s['total']}",
