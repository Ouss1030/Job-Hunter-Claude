...
CURRENT_STATUSES = {"NEW", "REACTIVATED", "UPDATED", "UNCHANGED"}
...
def load_pool(path):
    ...
    if str(payload.get("pool_version")) != "1.2":
        raise RuntimeError(
            f"{path}: pool_version={payload.get('pool_version')!r}, attendu 1.2."
        )
...
def compare_pools(current_path, previous_path):
    ...
    # Ordre du pool actuel pour NEW / UPDATED / UNCHANGED.
    for item in current["pool"]:
...
def validate_comparison(
    ...
    current_status_total = sum(
        summary[name]
        for name in ("NEW", "REACTIVATED", "UPDATED", "UNCHANGED")
    )
        ...
        reconstructed_previous = (
            {
                r["identity_key"]
                for r in records
                if r["delta_status"] in {"UPDATED", "UNCHANGED", "DISAPPEARED"}
            }
        )
...
def export_delta(result):
    ...
    for status in ("NEW", "REACTIVATED", "UPDATED", "DISAPPEARED"):
        rows = [
...
def print_summary(result, paths=None):
    ...
    print()
    print("✅ DELTA TRACKER V1.0 TERMINÉ.")
