...
ARTIFACT_PATTERNS = {
    "main_log": "job_hunter_main_v10_4_*.txt",
    "gate_json": "application_gate_v1_*.json",
    "queue_json": "application_queue_v1_*.json",
    "preparation_json": "application_preparation_v1_*.json",
    "preparation_txt": "application_preparation_v1_*.txt",
    "refresh_json": "job_refresh_v1_*.json",
    "refresh_txt": "job_refresh_v1_*.txt",
    "recheck_json": "application_recheck_v1_*.json",
    "recheck_txt": "application_recheck_v1_*.txt",
    "final_pool_json": "final_application_pool_v12_*.json",
    "final_pool_txt": "final_application_pool_v12_*.txt",
    "final_pool_csv": "final_application_pool_v12_*.csv",
    "delta_json": "delta_tracker_v1_*.json",
    "delta_txt": "delta_tracker_v1_*.txt",
    "delta_csv": "delta_tracker_v1_*.csv",
}
...
def run_preflight():
    ...
    for spec, expected in EXPECTED_VERSIONS.items():
        ...
        try:
            ...
            if actual != expected:
                errors.append(
                    f"Version incompatible {spec}: "
                    f"installé={actual}, attendu={expected}"
                )
...
def print_preflight(result):
    print("=" * 100)
    print("JOB HUNTER DAILY RUN V1.1 - PREFLIGHT")
    print("=" * 100)
    print()
    ...
    print()
    if result["ok"]:
        print("✅ PREFLIGHT DAILY RUN V1.1 VALIDÉ.")
    else:
...
def validate_preparation_json(path, limit):
    ...
    if gate_versions and gate_versions != {"1.3.2"}:
        raise RuntimeError(
            f"Preparation Gate incompatible: {sorted(gate_versions)}"
        )
    if queue_versions and queue_versions != {"1.2"}:
        raise RuntimeError(
...
def validate_final_pool_json(path):
    ...
    if str(payload.get("pool_version")) != "1.2":
        raise RuntimeError(
            f"Final Pool version={payload.get('pool_version')!r}, attendu 1.2."
        )
...
def write_manifest_txt(path, manifest):
    lines = []
    lines.append("JOB HUNTER DAILY RUN V1.1")
    lines.append("=" * 100)
...
def normalize_manifest_for_resume(manifest):
    ...
    compatible_versions = {"1.0", "1.0.1", "1.0.2", "1.1.0"}
    ...
    # Migration des manifests V1.0.x : ajoute l'étape Delta sans
    # invalider les étapes métier déjà DONE.
...
def print_final_summary(manifest):
    print()
    print("=" * 100)
    print("JOB HUNTER DAILY RUN V1.1 - BILAN")
    print("=" * 100)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Job Hunter Belgium Daily Run V1.1"
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--no-handoff",
        action="store_true",
        help="Exécuter jusqu’au Delta puis ignorer le Handoff.",
    )
    parser.add_argument(
        "--stop-after",
        choices=STEP_ORDER,
