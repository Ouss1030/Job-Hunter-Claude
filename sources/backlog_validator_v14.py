...
def collect_backlog_source_jobs(key):
    ...
    # V23_MANUAL_COMPLETION_DELEGATE
    try:
        from sources.manual_completion_v23 import collect_v23_source
        recovered_v23=collect_v23_source(key)
        if recovered_v23 is not None:
            return recovered_v23
    except Exception as exc:
        print(key,"V23 FALLBACK",type(exc).__name__,exc)
