"""
JOB HUNTER BELGIUM
PFIZER BELGIUM - VERSION 1.0

Direct-employer collector using Pfizer's public Workday careers endpoints.
...
CACHE_DIR = PROJECT_ROOT / "logs" / "pfizer_workday_cache"

CLIENT = WorkdayClient(
    host="https://pfizer.wd1.myworkdayjobs.com",
    tenant="pfizer",
    site="PfizerCareers",
    public_locale="en-US",
    cache_dir=CACHE_DIR,
...
TARGET_TITLE_PATTERNS = (
    ...
    '\\bquality\\s+(?:analyst|technician|specialist|associate|officer)\\b',
...
def _paginate(search_text: str = "", applied_facets: dict | None = None, label: str = "SEARCH"):
    ...
    while page < MAX_PAGES:
        ...
        page += 1
        print(
            f"PFIZER - {label:<18} page {page:2d}: {len(rows):2d} offre(s) "
...
def collect_pfizer_listing_candidates() -> tuple[list[dict], dict]:
    print("PFIZER - collecte directe Belgique via Workday public")
    found: dict[str, dict] = {}
    errors: list[str] = []
    strategy = []
    ...
    for location_query in ("Puurs", "Brussels", "Elsene"):
    ...
    for term in SEARCH_TERMS:
        ...
        after = len(found)
        print(f"PFIZER - mot-clé {term:<24}: {len(rows):2d} résultat(s) | +{after-before} Belgique")
...
def get_pfizer_job_detail(
    ...
    if not path:
        return {
            ...
            "structured": fallback,
            "from_cache": False,
            "error": "Pfizer : externalPath Workday introuvable.",
        }
    ...
    if not payload:
        return {
            ...
            "error": f"Pfizer Workday : {error or 'lecture détail impossible'}",
        }
...
def _job_from_detail(detail: dict, fallback: dict) -> JobOffer:
    ...
    job.collection_channel = "PFIZER"
    job.origin_source = "PFIZER"
    ...
    job.pfizer_requisition_id = clean_text(structured.get("job_requisition_id") or structured.get("external_id"))
    return job


def collect_pfizer_jobs() -> list[JobOffer]:
    _reset_geo_runtime_counters()
    candidates, meta = collect_pfizer_listing_candidates()
    print(
        "PFIZER - préfiltre : "
    ...
    for error in meta.get("errors") or []:
        print(f"PFIZER - avertissement : {error}")
    ...
    publish_metrics_from_locals("PFIZER", locals())
    return jobs
