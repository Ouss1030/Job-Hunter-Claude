"""
JOB HUNTER BELGIUM
CONNECTEUR SUCCESSFACTORS - TEST LIVE

    python -m diagnostics.successfactors_v1_live_test

⚠️ Ce script appelle l'API publique SuccessFactors. Il est classé en catégorie
...
from config.successfactors_sources import enabled_companies
...
def main():
    print("=" * 92)
    print("CONNECTEUR SUCCESSFACTORS - TEST LIVE")
    print("=" * 92)

    resultat = collect_successfactors_jobs(verbose=True)
    jobs = resultat["jobs"]
    ...
    # SuccessFactors n'expose pas de champ corporate séparé (contrairement à
    ...
    alertes = [r for r in rapport if r.get("layout_warning")]
    tests.append(check(
        "Aucune alerte de changement de gabarit",
        not alertes,
    ...
    (LOG_DIR / f"successfactors_v1_live_test_{stamp}.json").write_text(
        json.dumps({
    ...
    if passed != total:
        raise SystemExit("[FAIL] TEST LIVE SUCCESSFACTORS NON VALIDÉ.")

    print("[PASS] TEST LIVE SUCCESSFACTORS VALIDÉ.")
