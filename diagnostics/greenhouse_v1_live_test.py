"""
JOB HUNTER BELGIUM
CONNECTEUR GREENHOUSE - TEST LIVE

    python -m diagnostics.greenhouse_v1_live_test

⚠️ Ce script appelle l'API publique Greenhouse. Il est classé en catégorie
...
from config.greenhouse_sources import enabled_companies
...
def main():
    print("=" * 92)
    print("CONNECTEUR GREENHOUSE - TEST LIVE")
    print("=" * 92)

    resultat = collect_greenhouse_jobs(verbose=True)
    jobs = resultat["jobs"]
    ...
    # Greenhouse n'expose pas de champ corporate séparé (contrairement à
    ...
    tests.append(check(
        "Langue de publication exposée",
        all(hasattr(j, "posting_language") for j in jobs),
        "permet de repérer les annonces néerlandophones",
    ))
    ...
    (LOG_DIR / f"greenhouse_v1_live_test_{stamp}.json").write_text(
        json.dumps({
    ...
    if passed != total:
        raise SystemExit("[FAIL] TEST LIVE GREENHOUSE NON VALIDÉ.")

    print("[PASS] TEST LIVE GREENHOUSE VALIDÉ.")
